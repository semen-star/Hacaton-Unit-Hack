  ...
  let adminState = {
    ...
    eventQueue: [],      // events that will be processed asynchronously
  ...
  function processQueue() {
    ...
    toProcess.forEach(ev => {
      ...
      // тут реальная логика применения правил автоматизации
      applyAutomation(ev);
    });
    renderQueue();
  ...
  function renderColumns() {
    ...
    container.innerHTML = adminState.columns.map(col => `
      <div class="col-item">
        <div><span style="font-size:16px;">${col.emoji || "📌"}</span> <strong>${col.title}</strong> ${col.wipLimit ? `(WIP: ${col.wipLimit})` : ""}</div>
        <div>
    ...
    document.getElementById("flowPreview").innerHTML = adminState.columns.map(c => `${c.emoji} ${c.title}`).join(" → ");
  }
  ...
  function renderRules() {
    ...
    container.innerHTML = adminState.automationRules.map(rule => `
      <div class="rule-item">
        <div><strong>${rule.name}</strong><br><span class="rule-badge">${rule.trigger} → ${rule.action}</span></div>
        <div>
  ...
  function renderAutoLog() {
    ...
    logDiv.innerHTML = adminState.autoLogs.map(l => `<div class="log-entry">[${l.time}] ${l.rule} → ${l.event}</div>`).join("");
  }
  ...
  function editColumn(id) {
    ...
    document.getElementById("modalBody").innerHTML = `
      <div class="form-group"><label>Название:</label><input id="colTitle" value="${col.title}"></div>
  ...
  function toggleMenu(el) { document.querySelectorAll(".menu-item.open").forEach(m => m.classList.remove("open")); if(!el.classList.contains("open")) el.classList.add("open"); else el.classList.remove("open"); }
  ...
  function showAbout() { alert("KanbanOS 95 Admin Panel\nВерсия 1.0\nEvent-driven архитектура\nГотов к интеграции с RabbitMQ/Kafka\n\n© 1995-2025"); }
  function updateClock() { const d = new Date(); document.getElementById("clock").innerText = d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}); }
  setInterval(updateClock, 1000); updateClock();
  document.addEventListener("click", (e) => { if(!e.target.closest(".menu-bar")) document.querySelectorAll(".menu-item.open").forEach(m => m.classList.remove("open")); if(!e.target.closest(".start-menu") && !e.target.closest(".start-btn")) document.getElementById("startMenu").classList.remove("open"); });
  loadFromLocal();